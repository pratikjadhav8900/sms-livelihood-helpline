...
export default function Analytics() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/analytics/dashboard?days=${timeRange}`, {
        headers: {
  ...
  const fetchIntents = async () => {
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`http://localhost:8080/api/analytics/intents?days=${timeRange}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  const fetchPerformance = async () => {
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`http://localhost:8080/api/analytics/performance?days=${timeRange}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
