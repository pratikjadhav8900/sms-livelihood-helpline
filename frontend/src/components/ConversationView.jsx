...
export default function ConversationView({ ticket, onUpdate }) {
  ...
  const fetchTicketDetails = async () => {
    try {
      setLoading(true)
      const response = await axios.get(`http://localhost:8080/api/tickets/${ticket.id}`)
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      await axios.post(`http://localhost:8080/api/tickets/${ticket.id}/reply`, {
  ...
  const handleStatusChange = async (newStatus) => {
    try {
      await axios.put(`http://localhost:8080/api/tickets/${ticket.id}/status`, {
        status: newStatus
      })
      onUpdate()
    } catch (error) {
