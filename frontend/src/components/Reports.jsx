...
const Reports = ({ token }) => {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/reports/analytics?days=${dateRange}`, {
        headers: {
  ...
  const exportToCSV = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/reports/tickets/export?${params.toString()}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
